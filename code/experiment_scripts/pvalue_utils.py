...
def pool_analysis(parameters, covariances, return_pvalues=True):
    ...
    cov_within = np.nanmean(covariances, axis=1)

    # The between-imputation covariance
    cov_between = np.nanvar(parameters, axis=1)
    ...
    assert np.all(np.isnan(T_m) | (T_m > 0)), f"{T_m=} {T_m_pos=}"
    ...
    if return_pvalues:
        ...
        return Q_m, pd.Series(T_m, index=index)
